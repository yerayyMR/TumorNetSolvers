...
def unpack_dataset(folder: str, unpack_segmentation: bool = True, overwrite_existing: bool = False,
    ...
    """
    Unpacks all npz files in the folder, optionally using multiprocessing.
    """
    npz_files = subfiles(folder, True, None, ".npz", True)

    # Debugging: Print the files being processed
    print(f"Found {len(npz_files)} npz files to unpack.")

    # Temporarily run without multiprocessing to see if it works sequentially
    for npz_file in npz_files:
        try:
            _convert_to_npy(npz_file, unpack_segmentation, overwrite_existing, verify_npy)
            print(f"Successfully unpacked {npz_file}")
        except Exception as e:
            print(f"Failed to unpack {npz_file}: {e}")
