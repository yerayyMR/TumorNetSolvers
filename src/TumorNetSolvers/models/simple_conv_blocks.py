...
class StackedConvBlocks(nn.Module):
    def __init__(self,
                 ...
                 concat: bool = False,
                 current_input_shape: List[int] = None
                 ):
        """
        ...
        # First convolution with initial_stride
        convs = [
            ConvDropoutNormReLU(
        ...
        # Prepare input channels for the second conv + create linear layer
        next_in_channels = output_channels[0]
        if (self.param_dim > 0 and self.linearLayer):
            if self.concat:
                self.param_fcs = nn.Linear(in_features=param_dim, out_features=(current_input_shape//initial_stride[0])**3 * self.param_dim)
                next_in_channels += self.param_dim
            elif not self.concat:
                self.param_fcs = nn.Linear(in_features=param_dim, out_features=(current_input_shape//initial_stride[0])**3 * next_in_channels)
        self.next_in_channels = next_in_channels
        # Remaining convolutions (always stride=1)
        for i in range(1, num_convs):
            convs.append(
                ConvDropoutNormReLU(
                    conv_op, next_in_channels, output_channels[i], kernel_size, 1, conv_bias,
                    norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs, nonlin_first
                )
            )
            next_in_channels = output_channels[i]
    ...
    def forward(self, x: torch.Tensor, param: Optional[torch.Tensor] = None) -> torch.Tensor:
        x = self.convs[0](x)  # This is the downsampling conv

        if self.param_dim > 0 and self.linearLayer and param is not None:
            B = x.shape[0]  # batch size
            spatial_shape = x.shape[2:]  # e.g., (D, H, W)

            param_proj = self.param_fcs(param)  # (B, N)
            param_proj = param_proj.view(B, -1, *spatial_shape)  # reshape to match spatial dims
            if self.concat:
